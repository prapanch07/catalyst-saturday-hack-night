    ...
    def route_change(e: ft.RouteChangeEvent):
        if e.route == "/card":
            ...
            def button_clicked(e):
                ...
                timer =  ft.Slider(width=300,value=5,min=0,max=20,divisions=10,label="{value}min")
                page.add(timer)
                page.add(ft.IconButton(ft.icons.ARROW_FORWARD_IOS_ROUNDED,on_click=lambda _:page.go("/music")))
                route_data['timer_value'] = timer.value
                page.update()
            ...
            timer_value = route_data.get('timer_value')
            def button_clicked(e):
                ...
                if dd.value == "Nature":
                        ...
                        def get_nature():
                            async def countdown():
                                for i in range(timer_value, 0, -1):  # 180 seconds countdown (3 minutes)
